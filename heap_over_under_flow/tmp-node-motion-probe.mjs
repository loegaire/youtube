...
const makeElement = (name) => ({
  ...
  addEventListener() {},
  removeEventListener() {},
  attachShadow() { this.shadowRoot = makeElement('shadow-root'); return this.shadowRoot; },
...
globalThis.AudioContext = class { decodeAudioData() { return Promise.reject(new Error('Audio is disabled for video-only rendering.')); } };
...
const target = Math.min(player.duration.current - 1, 120);
player.requestSeek(target);
player.requestRender();
for (let attempt = 0; attempt < 2000 && (player.frame.current !== target || renderCount === 0); attempt++) {
  await new Promise((resolve) => setTimeout(resolve, 5));
}
writeFileSync('output/node-probe.png', stage.finalBuffer.toBuffer('image/png'));
console.log('frame', player.frame.current, 'renders', renderCount);
player.deactivate();
